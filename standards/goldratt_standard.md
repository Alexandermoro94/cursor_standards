...
**Input:**
- Текстовый документ с описанием производственного процесса (описание текущего процесса производства, этапы, оборудование, персонал, например: "Производство включает 5 этапов: заготовка (станок А, 2 оператора), обработка (станок Б, 1 оператор), сборка (линия из 3 рабочих мест), покраска (камера, 1 оператор), упаковка (конвейер, 2 оператора). Работает 3 смены по 8 часов, 5 дней в неделю")
- Данные о производительности системы (метрики Throughput, Inventory, Operating Expense за последний период, например: "Throughput: $50,000/месяц (продажи $100,000 минус переменные затраты $50,000), Inventory: $200,000 (сырье $80,000 + НЗП $70,000 + готовая продукция $50,000), Operating Expense: $30,000/месяц (зарплата $20,000 + аренда $7,000 + коммунальные $3,000)")
- Данные о загрузке оборудования и ресурсов (время работы каждого станка/линии, простои, загрузка персонала, например: "Станок А работает 95% времени (простои только на обслуживание), Станок Б - 60% времени (часто простаивает из-за отсутствия заготовок), линия сборки - 80% (ожидает детали от Станка Б), покрасочная камера - 50% (недогружена)")
- Данные о спросе и заказах (объем заказов, сроки выполнения, невыполненные заказы, например: "Входящие заказы: 1000 единиц/месяц, выполняем: 800 единиц/месяц, накоплено невыполненных: 500 единиц, средняя задержка выполнения: 5 дней")
- Ограничения и требования (технические ограничения, требования к качеству, доступные ресурсы, например: "Бюджет на изменения: $10,000, нельзя останавливать производство более чем на 1 день, требования к качеству: дефектность <1%, нельзя сокращать персонал")

**Output:**
- Текстовый документ с анализом ограничений (идентифицированное ограничение системы, тип ограничения - физическое/политическое/парадигма, влияние на производительность, например: "Ограничение: Станок Б на этапе обработки, тип: физическое (мощность станка 10 единиц/час при требуемых 15 единиц/час), влияние: ограничивает общую производительность на 40%, создает очередь из 200 единиц НЗП перед этапом сборки")
- План применения пяти фокусирующих шагов (структурированный план действий: как эксплуатировать ограничение, как подчинить систему, как поднять ограничение, например: "Шаг 1: Максимизировать использование Станка Б - убрать простои (обеспечить непрерывную подачу заготовок, убрать перерывы на обед для оператора), Шаг 2: Синхронизировать все этапы с ритмом Станка Б (заготовка производит ровно 10 единиц/час, сборка ждет 10 единиц/час), Шаг 3: Поднять ограничение - добавить второй оператор и вторую смену на Станок Б")
- Метрики до и после оптимизации (сравнительная таблица: Throughput, Inventory, Operating Expense до и после, например: "Throughput: было $50,000/месяц, стало $75,000/месяц (+50%), Inventory: было $200,000, стало $150,000 (-25%), Operating Expense: было $30,000/месяц, стало $32,000/месяц (+7% из-за дополнительного оператора), время цикла: было 5 дней, стало 3 дня (-40%)")
- Отчет о результатах оптимизации (описание внесенных изменений, достигнутые результаты, новые ограничения, например: "Добавлен второй оператор на Станок Б, организована вторая смена, Throughput увеличен на 50%, Inventory снижен на 25%, новое ограничение: этап покраски (не успевает обрабатывать возросший поток), ROI: 400% за 3 месяца")
...
**Input:**
- Текстовый документ с описанием проекта (описание целей проекта, основные задачи, заинтересованные стороны, например: "Проект: разработка мобильного приложения для iOS и Android, цель: автоматизация заказа товаров, срок: 3 месяца, команда: 5 разработчиков (2 iOS, 2 Android, 1 backend), 2 QA инженера, 1 дизайнер, заинтересованные стороны: продукт-менеджер, CEO, отдел продаж")
- Сетевой график проекта с зависимостями (список задач с зависимостями, оценки времени, ответственные, например: "Задача 1: Дизайн UI/UX (5 дней, дизайнер) → Задача 2: Разработка Backend API (10 дней, backend разработчик) → Задача 3: Разработка iOS приложения (15 дней, iOS разработчики) → Задача 4: Разработка Android приложения (15 дней, Android разработчики) → Задача 5: Интеграционное тестирование (5 дней, QA)")
- Данные о ресурсах проекта (количество разработчиков, их специализация, доступность, например: "2 backend разработчика (100% доступность, работают только на этом проекте), 2 iOS разработчика (80% доступность, 20% времени на поддержку других проектов), 2 Android разработчика (80% доступность), 1 fullstack (50% доступность, разделен между 2 проектами), 2 QA инженера (100% доступность), 1 дизайнер (100% доступность)")
- История предыдущих проектов (данные о срывах сроков, причины задержек, средние оценки времени, например: "Последние 3 проекта: средняя задержка 30% (проект А: +25%, проект Б: +35%, проект В: +30%), причины: завышенные оценки времени (в среднем на 50%), студенческий синдром (работа начиналась за 2 дня до дедлайна), конфликты ресурсов (fullstack разработчик перегружен), непредвиденные технические проблемы")
- Требования к проекту (дедлайн, бюджет, качество, например: "Дедлайн: 1 марта 2025, бюджет: $100,000, качество: покрытие тестами >80%, производительность: время загрузки <2 секунд, совместимость: iOS 14+, Android 8+")

**Output:**
- Сетевой график с критической цепочкой (визуальная схема проекта с выделенной критической цепочкой, учетом ограничений ресурсов, например: "Критическая цепочка: Дизайн (5 дней) → Backend API (10 дней) → iOS разработка (15 дней, учитывая 80% доступность = 18.75 дней) → Интеграционное тестирование (5 дней), общая длительность: 38.75 дней. Параллельные цепочки: Android разработка (18.75 дней) имеет Feeding Buffer 3 дня")
- План управления буферами (размеры Project Buffer, Feeding Buffer, Resource Buffer, правила мониторинга, например: "Project Buffer: 12 дней (30% от критической цепочки 38.75 дней), Feeding Buffer: 3 дня для Android цепочки (защищает критическую цепочку от задержек в параллельной разработке), Resource Buffer: 1 день для fullstack разработчика (обеспечивает доступность при конфликтах), мониторинг: ежедневно в 10:00, отчеты: еженедельно")
- Система мониторинга буферов (инструмент или таблица для отслеживания использования буферов в реальном времени, например: "Dashboard в Jira с метриками: Project Buffer Penetration: 25% (использовано 3 дня из 12), Feeding Buffer Penetration: 15% (использовано 0.45 дня из 3), Critical Chain Progress: 60% (выполнено 23.25 дня из 38.75), тренд: стабильный, прогноз завершения: 28 февраля (на 1 день раньше дедлайна)")
- Отчет о результатах проекта (сравнение плановых и фактических сроков, использование буферов, уроки, например: "Проект завершен на 2 дня раньше срока (28 февраля вместо 1 марта), Project Buffer использован на 20% (2.4 дня из 12), Feeding Buffer использован на 10% (0.3 дня из 3), критическая цепочка не нарушена, уроки: агрессивные оценки времени (50% от традиционных) оказались реалистичными, централизованные буферы эффективно защитили проект от задержек")
- Протокол управления проектом (документ с правилами принятия решений на основе состояния буферов, например: "Если Project Buffer <50% - норма (зеленая зона, продолжаем работу), 50-75% - внимание (желтая зона, анализируем причины, готовим план действий), >75% - критично (красная зона, активируем план восстановления, добавляем ресурсы), аналогично для Feeding Buffer")
...
**Input:**
- Текстовый документ с описанием бизнес-процессов клиента (описание текущих процессов, этапы, участники, например: "Процесс обработки заказов: прием заказа (менеджер по продажам, 30 мин) → проверка наличия товара (складской работник, ручная проверка в Excel, 2 часа) → оплата (бухгалтер, проверка платежа, 1 день) → сборка заказа (склад, 4 часа) → доставка (курьер, 1 день), среднее время цикла: 5 дней, обрабатывается 200 заказов/месяц")
- Данные о метриках системы (Throughput, Inventory, Operating Expense, время цикла процессов, например: "Throughput: $200,000/месяц (продажи $300,000 минус переменные затраты $100,000), Inventory: $500,000 (товары на складе $400,000 + НЗП $50,000 + готовая продукция к отгрузке $50,000), Operating Expense: $150,000/месяц (зарплата $100,000 + аренда $30,000 + коммунальные $20,000), время цикла: 5 дней, Inventory Turns: 0.4 оборота/месяц")
- Данные о проблемах и жалобах (описание проблем, частота возникновения, влияние на бизнес, например: "Проблема 1: задержки доставки, частота: 30% заказов (60 из 200), средняя задержка: 2 дня, влияние: потеря 15 клиентов/месяц, ущерб репутации, жалобы в соцсетях. Проблема 2: ошибки в проверке наличия, частота: 10% заказов, влияние: отмена заказов, недовольство клиентов")
- Карта процессов (визуальная схема процессов с этапами, временем выполнения, зависимостями, например: "Схема процесса в BPMN формате: Start → Прием заказа (30 мин, менеджер) → Проверка наличия (2 часа, склад, блокер: ручная проверка в Excel) → Оплата (1 день, бухгалтер) → Сборка (4 часа, склад) → Доставка (1 день, курьер) → End. Параллельные процессы: уведомление клиента (автоматически после оплаты)")
- Ограничения и требования клиента (бюджет на изменения, ограничения по времени, требования к результату, например: "Бюджет: $50,000, срок оптимизации: 2 месяца, нельзя сокращать персонал более чем на 10%, нельзя останавливать процесс обработки заказов, требования: сократить время цикла на 40%, увеличить Throughput на 30%, снизить количество ошибок до <1%")

**Output:**
- Текстовый документ с анализом ограничений системы (идентифицированные ограничения, их тип и влияние, например: "Ограничение 1: этап проверки наличия товара, тип: политическое (требуется ручное подтверждение складским работником в Excel, нет интеграции с системой учета), влияние: увеличивает время цикла на 2 дня (2 часа работы + 1.5 дня ожидания подтверждения), создает очередь из 40 заказов. Ограничение 2: этап оплаты, тип: политическое (бухгалтер проверяет каждый платеж вручную), влияние: задержка 1 день")
- План оптимизации процессов (структурированный план применения пяти фокусирующих шагов, приоритизированные действия, например: "Шаг 1: Идентифицировать ограничение - этап проверки наличия (политическое). Шаг 2: Эксплуатировать ограничение - автоматизировать проверку наличия через интеграцию с системой учета, убрать ручное подтверждение. Шаг 3: Подчинить систему - синхронизировать все этапы с новым ритмом (проверка наличия теперь 5 минут вместо 2 часов), оптимизировать этап оплаты (автоматическая проверка платежей). Шаг 4: Поднять ограничение - внедрить систему автоматической проверки наличия в реальном времени")
- Метрики до и после оптимизации (сравнительная таблица с метриками системы, например: "Throughput: было $200,000/месяц, стало $280,000/месяц (+40%, обрабатывается 280 заказов вместо 200), Inventory: было $500,000, стало $350,000 (-30%, снижение НЗП и готовой продукции), Operating Expense: было $150,000/месяц, стало $145,000/месяц (-3%, оптимизация процессов), время цикла: было 5 дней, стало 3 дня (-40%), Inventory Turns: было 0.4, стало 0.8 оборота/месяц (+100%)")
- Отчет о результатах оптимизации (описание внесенных изменений, достигнутые результаты, ROI, например: "Внедрена автоматизация проверки наличия через интеграцию с системой учета (время проверки: 2 часа → 5 минут), автоматизирована проверка платежей (время: 1 день → 1 час), Throughput увеличен на 40%, время цикла сокращено на 40%, количество ошибок снижено с 10% до 0.5%, ROI: 300% за 6 месяцев (инвестиции $50,000, экономия $150,000/год)")
- Рекомендации по дальнейшей оптимизации (план следующих шагов, новые ограничения, возможности улучшения, например: "Следующее ограничение: этап сборки заказа (4 часа, можно оптимизировать до 2 часов через улучшение логистики склада), рекомендация: внедрить систему оптимизации маршрутов сборки, использовать штрих-коды для ускорения поиска товаров, потенциальное увеличение Throughput еще на 15%")
